# import pygame
# from gameconfig import Gameconfig
# import time 
# from background import BG

# class Player(pygame.sprite.Sprite):
#     LEFT = -1
#     RIGHT = 1
#     NONE = 0
    
#     def __init__(self, x):
#         pygame.sprite.Sprite.__init__(self)
#         self.rect = pygame.Rect(x, 200, Gameconfig.PLAYER_W, Gameconfig.PLAYER_H)
#         self.sprite_count = 0
#         self.direction = Player.NONE 
#         self.image = Player.IMAGES[self.direction][self.sprite_count//Gameconfig.NB_FRAMES_PER_SPRITE_PLAYER]
#         self.mask = Player.MASKS[self.direction][self.sprite_count//Gameconfig.NB_FRAMES_PER_SPRITE_PLAYER]
#         self.vx = 0
#         self.vy = 0
#         self.count_cle = 0
#         self.on_ground = True
    
#     def draw(self, window, seuil):
#         screen_x = self.rect.x + seuil
#         if screen_x < 0:
#             screen_x = 0
#         if screen_x > Gameconfig.WINDOW_W - Gameconfig.PLAYER_W:
#             screen_x = Gameconfig.WINDOW_W - Gameconfig.PLAYER_W
#         window.blit(self.image, (screen_x, self.rect.y))

#     def advance_state(self, next_move, platforms):
#         # ---- 1. GESTION DE L'AXE HORIZONTAL (X) ----
#         fx = 0
#         if next_move.left:
#             fx = Gameconfig.FORCE_LEFT
#             self.direction = Player.LEFT
#         elif next_move.right:
#             fx = Gameconfig.FORCE_RIGHT    
#             self.direction = Player.RIGHT 
#         else:
#             self.direction = Player.NONE

#         x, y = self.rect.topleft
#         vx_min = -x / Gameconfig.DT
#         vx_max = 1000  
#         self.vx = fx * Gameconfig.DT
#         self.vx = min(self.vx, vx_max)
#         self.vx = max(self.vx, vx_min)
    
#         # Déplacement horizontal
#         self.rect.x += self.vx * Gameconfig.DT
        
#         # Résolution des collisions horizontales (murs)
#         for platforme in platforms:
#             if self.rect.colliderect(platforme):
#                 if self.vx > 0:  # Se déplace vers la droite
#                     self.rect.right = platforme.left
#                 elif self.vx < 0:  # Se déplace vers la gauche
#                     self.rect.left = platforme.right
#                 self.vx = 0  # On stoppe la vitesse horizontale

#         # ---- 2. GESTION DE L'AXE VERTICAL (Y) ----
#         if next_move.jump and self.on_ground:
#             fy = Gameconfig.FORCE_JUMP
#             self.vy = fy * Gameconfig.DT
#             self.on_ground = False  # Quitte le sol suite au saut
#         else:
#             self.vy = self.vy + Gameconfig.GRAVITY * Gameconfig.DT  
        
#         # Déplacement vertical
#         self.rect.y += self.vy * Gameconfig.DT
        
#         # Résolution des collisions verticales (sol / plafond)
#         self.on_ground = False  # On suppose qu'il tombe tant qu'on n'a pas prouvé qu'il touche un sol
#         for platforme in platforms:
#             if self.rect.colliderect(platforme):
#                 if self.vy > 0:  # En train de tomber (touche un sol)
#                     self.rect.bottom = platforme.top
#                     self.vy = 0
#                     self.on_ground = True
#                 elif self.vy < 0:  # En train de monter (touche un plafond)
#                     self.rect.top = platforme.bottom
#                     self.vy = 0

#         # ---- 3. ANIMATION DU SPRITE ----
#         if self.sprite_count >= Gameconfig.NB_FRAMES_PER_SPRITE_PLAYER * len(Player.IMAGES[self.direction]): 
#             self.sprite_count = 0
#         time.sleep(0.001) 
#         num_images = len(Player.IMAGES[self.direction])
#         frame_index = (self.sprite_count // Gameconfig.NB_FRAMES_PER_SPRITE_PLAYER) % num_images
#         self.image = Player.IMAGES[self.direction][frame_index]
#         self.mask = Player.MASKS[self.direction][frame_index]
#         self.sprite_count += 1

#     @staticmethod
#     def init_sprites():
#         Player.IMAGES = {Player.LEFT : Gameconfig.WALK_LEFT_IMG, Player.RIGHT : Gameconfig.WALK_RIGHT_IMG, Player.NONE : Gameconfig.STANDING_IMG}
#         Player.MASKS = {Player.LEFT : Gameconfig.WALK_LEFT_MASKS, Player.RIGHT : Gameconfig.WALK_RIGHT_MASKS, Player.NONE : Gameconfig.STANDING_MASK}